// Google Apps Script only supports tooltips in libraries' global-level
// declarations.
// https://issuetracker.google.com/issues?q=title:jsdoc
 ...
 * グループチャットの概要
 *
 * @param {boolean | 1 | 0} [formData.link=false]
 * **任意**
 * 招待リンクを作成するか
 *
 ...
 *
 * @param {boolean | 1 | 0} [formData.link_need_acceptance=true]
 * **任意**
 * 参加に管理者の承認を必要とするか
 *
...
function postRooms(token, formData) {
  ...
  if (!formData.link) {
    formData.link = 0;
  } else if (typeof formData.link === "boolean") {
    formData.link = formData.link ? 1 : 0;
  }

  if (!formData.link_need_acceptance) {
    formData.link_need_acceptance = 1;
  } else if (typeof formData.link_need_acceptance === "boolean") {
    formData.link_need_acceptance = formData.link_need_acceptance ? 1 : 0;
  }
 ...
 * @return {Message[]}
 */
function getRoomMessages(token, roomId, queryParams = { force: false }) {
  if (!roomId) {
    throw new MissingParameterError_("roomId");
  }
...
function getRoomTasks(token, roomId, queryParams) {
  ...
  if (queryParams && !isValidTaskStatus_(queryParams.status)) {
    console.warn(
 ...
 *
 *
 * @param {boolean | 1 | 0} [queryParams.create_download_url]
 * **任意**
 *
 * ダウンロードURLを作成するか。
 ...
 * 省略するとランダムな文字列となります。
 *
 * @param {boolean | 1 | 0} formData.need_acceptance
 * 参加に管理者の承認を必要とするか
 *
 * @param {string} formData.description
 * 招致リンクのページに表示される説明文
...
function isValidTaskStatus_(taskStatus) {
  return !!taskStatus && /^(open|done)$/i.test(taskStatus);
}
...
class ChatworkError_ extends Error {
   ...
   * @arg {"メッセージ" | "タスク" | "ファイル" | "コンタクト" | "リソース"} [resourceType="リソース"]
   * **任意**
   */
...
/**
 * @typedef {object} TaskIdResponse
 *
 * @prop {string} TaskIdResponse.task_id
 */
...
/**
 * @typedef {"none" | "date" | "time"} TaskLimitType
 */
 ...
 *
 * @prop {string[]} UploadedFiles.file_ids
 */
